...

{
  ...
  "include_keywords": [
    "unambiguous keyword or phrase",
    ["ambiguous title", "industry qualifier"]
  ],
...
Note the two kinds of include_keywords entries:
- a plain string matches on its own (use for titles/terms that are
  specific enough to identify my field by themselves)
- a LIST of strings matches only when ALL its parts appear somewhere in
  the same posting, in any order (use for generic, cross-industry titles
  that would otherwise flood me with postings from unrelated fields —
  e.g. ["project manager", "construction"] instead of the bare
...
5. Build include_keywords: exact job titles + synonyms (including other
   ...
   characteristic of the profession. For each title, honestly ask
   yourself: does this phrase identify my field BY ITSELF, or does it
   exist across many industries? Field-specific titles go in as plain
   strings. Generic cross-industry titles (e.g. "product manager",
   "project manager", "analyst", "account manager") must go in as
   AND-lists paired with an industry qualifier — ["product manager",
   "automotive"] — otherwise they will match postings from every
   industry that uses that title. Prefer short word stems for the
   qualifier where the language inflects words (e.g. Russian), so all
   grammatical cases match. Avoid overly generic single words that
   catch noise even within the right industry.
